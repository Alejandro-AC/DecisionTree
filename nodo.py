...
class Nodo():
    ...
    def _init_debugexiste(self):    ##funcion solo para debuggin
        ...
        for elemento in self.dataHijos:
            if (elemento[14]=='g') and (elemento[8]=='n'): #añadir en el if cualquier campo que quieras comprobar si existe o no
    ...
    def guany(self):       ###LA FUNCION NO VA CORRECTAMENTE  ----REVISAR-----
        ...
        if len(self.listaDicVen)>0:    ##comprobamos que las opciones que nos quedan con la variable pasada son venenossas
            for key in self.listaDicVen[0]:
                Total = Total + self.listaDicVen[0][key]
        if len(self.listaDicEd)>0:    ##lo mismo para las no venenosass, asi cuando ya no queda ninguna por tratar no se calcula la H
