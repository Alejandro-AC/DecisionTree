...
class DecisionAlgorithm:
    ...
    def calculate_gain(self, base=None, data_set=None, gini=None):
        data_set = self.data_set if data_set is None else data_set
        gini = False if gini is None else gini

        num_columns = data_set.shape[1]
        gain_list = [base] * num_columns  # Initialize with base value
        for col in range(1, num_columns):
            ...
            for value, count in zip(unique_values, counts):
                ...
                if gini:
                    gain_list[col] -= count / total_count * self.calculate_gini(value_rows[:, 0])
                else:
    ...
    def calculate_gini(self, class_values=None):
        class_values = self.data_set[:, 0] if class_values is None else class_values
        value, counts = np.unique(class_values, return_counts=True)
        norm_counts = counts / counts.sum()
